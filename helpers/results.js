...
function saveDailyResult(slug, payload) {
  ...
  if (!/^\d{4,5}$/.test(prize1)) {
    throw new Error('Prize 1 harus 4 atau 5 digit.');
  }
  ...
  const entry = {
    ...
    sourceMarket: payload.sourceMarket || null,
    sourceUrl: payload.sourceUrl || null,
    createdAt: new Date().toISOString()
  };

  // History lengkap selalu berisi result terbaru, termasuk hari ini.
  ...
  // latest adalah tanggal terbaru, bukan sekadar request terakhir.
  const candidates = [entry, resultPayload.latest].filter(Boolean);
  candidates.sort((a, b) => new Date(b.date) - new Date(a.date));
  resultPayload.latest = candidates[0] || entry;
